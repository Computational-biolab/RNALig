...
bundle = {
    "model": pipe,                 # sklearn Pipeline (imputer + RF)
    "features": feat_names,        # exact columns order
    "target": target_col,          # target column name
    "meta": {
        "n_train": int(len(y)),
        "random_state": SEED,
        "preprocessing": "median imputation + variance/duplicate filter",
        "note": "trained on full 164-PDB Training_data.csv (70% dataset); no calibration, no validation split"
    }
}
